...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.dryRun) {
    try {
      ...
      return NextResponse.json({ ok: true, testMode: lobIsTestMode(), lobConfigured: lobEnabled(), ...preview });
    } catch (err) {
  ...
  if (!lobEnabled()) return NextResponse.json({ error: "lob_not_configured" }, { status: 400 });
  ...
  return NextResponse.json({ ok: true, runId: handle.id, testMode: lobIsTestMode() });
}
