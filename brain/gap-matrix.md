...
| G-C2c | Med | Worker fill-schema size: rich multi-page templates can exceed Gemini constrained-decoding limits at slot-fill → non-retryable `AI_APICallError 400`. Hit on `hvac-precision-comfort` (REIC, 2026-06-16). Worker-side (sitelaunchr-builder); SL+Builder fixing. | [C2](./contracts/c2-sitelaunchr-builds-wr-template.md) | Industry IS supported — downstream render failure, not a vocabulary miss. |
