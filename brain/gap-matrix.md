...
| G-C4 | High → **partially closed** | Real build-cost attribution. **Template flow (2026-09-01): WR now consumes `cost_usd`/`usage` on the wr-template callback + reconcile replay, recording it in `tpl_cost_events` idempotent on `run_id=build_id` (ADR 0007; SL emits it as of sitelaunchr-builder #6).** End-to-end for the template flow is pending SL's `/api/internal/builds/status` validator accepting the 2 additive fields (item v). **Onboarding flow still open:** `build_jobs.cost_usd` remains a clamped duration estimate until SL emits cost on the `wr`-source callback. | [C4](./contracts/c4-sitelaunchr-callbacks.md) | Template-flow consumer shipped + no-op-ready; onboarding consumer awaits SL. |
