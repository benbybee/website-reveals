...
| G-C2c | High | Worker fill-schema size: `hvac-precision-comfort` builds non-retryably `AI_APICallError 400` ("too many states"). **PR #3 (chunked fill) INSUFFICIENT** — real paid build 2026-06-16 (360 HVAC PRO, worker run 27647122585) still fails: the `filling`-phase `generateObject` schema **still spans multiple pages** (`svcpage.*`+`aboutpage.*`) with long dotted keys + a ~52-val icon enum + `minLength` matchers. Their golden-run test passed but the real build fails (test gap). | [C2](./contracts/c2-sitelaunchr-builds-wr-template.md) | Worker-side (sitelaunchr-builder). Industry IS supported — downstream render failure. error_message synthesis (PR #7) verified to WR `build_error`. |
| G-C2d | Med | **Failed builds aren't retryable:** re-POST of a terminal-`failed` `external_id` returns the cached failure (`duplicate:true`), not a fresh build — so a lead stays stuck even after the underlying bug is fixed (REIC). SL API dedup treats `failed` as terminal-dedup. | [C2](./contracts/c2-sitelaunchr-builds-wr-template.md) | SL (API) fix: re-POST of a `failed` build should start a fresh build, or add a force/retry path. |
