...
| G-C4 | High → **partially closed** | Real build-cost attribution. **Template flow (2026-09-01): WR now consumes `cost_usd`/`usage` on the wr-template callback + reconcile replay, recording it in `tpl_cost_events` idempotent on `run_id=build_id` (ADR 0007; SL emits it as of sitelaunchr-builder #6).** End-to-end for the template flow is pending only SL's **C4 relay forwarding** `cost_usd`/`usage` to WR's callback URL (item 1a — tracked in SL-app issue benbybee/sitelaunchr#9); SL's validator **accepts** the fields (1b green, HTTP 200 verified). **Onboarding flow still open:** `build_jobs.cost_usd` remains a clamped duration estimate until SL emits cost on the `wr`-source callback. | [C4](./contracts/c4-sitelaunchr-callbacks.md) | Template-flow consumer shipped + no-op-ready; onboarding consumer awaits SL. |
...
| G-C2 | Med | No automated SL-side conformance probe; `brief.industry` must match an SL template slug (vocabulary now documented in [C2](./contracts/c2-sitelaunchr-builds-wr-template.md)). **WR pre-dispatch coverage gate now shipped** (2026-09-01, ADR 0007): `tpl_industries.sl_template_ready` drives the rep picker and `assembleAndPush` drops any prospect whose `industry_slug` isn't in the ready set (`skipped[…]`), so WR never ships a truly-unsupported industry. Residual: no live SL-side conformance probe (relies on SL `.strict()` reject). | [C2](./contracts/c2-sitelaunchr-builds-wr-template.md) | Failure feedback improved: SL synthesizes `error_message` on null-reason failures (PR #7). |
...
| G-C2e | Low | **SL template content fill — mostly fixed.** Original (REIC): placeholder service-area towns, empty blog-despite-teasers, empty `mailto:`, name-only JSON-LD. **Verified fixed on the 360 HVAC PRO rebuild 2026-06-16:** ✅ service area = real Denver-metro towns (Lakewood/Aurora/Thornton/…), zero placeholder leak, real testimonial locales; ✅ zero empty `tel:`/`mailto:` (email hidden when absent); ✅ blog teasers removed (no broken promise). **Still open (low, SEO-only, prospect-invisible):** LocalBusiness JSON-LD lacks `areaServed`, `og:description`/meta = bare business name, `PostalAddress` is unstructured (streetAddress-only); `/blog` is an orphaned empty shell (consider noindex). | [C2](./contracts/c2-sitelaunchr-builds-wr-template.md) | sitelaunchr-builder. Prospect-facing content now correct; remainder is structured-data polish. |
...
| G-BUD1 | Med | No max-cost/budget gate per **campaign** (bulk L2 Apify spend). A misconfigured campaign (high `target_count` × many locations) can incur unbounded spend; cost events are recorded **post-facto**. **Partially addressed for the rep instant-preview arm** (2026-09-01, ADR 0007): a per-rep/day USD cap (`REP_DAILY_BUDGET_USD`, `withinRepBudget`) gates that interactive flow before any paid Places call — but it's a read-then-act soft cap (concurrent confirms could marginally exceed it) and does not cover bulk campaigns. | L2 |
