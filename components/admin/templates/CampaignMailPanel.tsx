...
type Provider = "lob" | "click2mail" | "export";

const PROVIDER_LABEL: Record<Provider, string> = {
  lob: "Lob",
  click2mail: "Click2Mail",
  export: "Export (manual CSV)",
};
...
export function CampaignMailPanel({
  ...
  const [provider, setProvider] = useState<Provider>((initialProvider as Provider) ?? "lob");
  const [savingField, setSavingField] = useState<null | "design" | "address" | "provider">(null);
  ...
  async function assign(field: "design" | "address" | "provider", value: string) {
    setSavingField(field);
    setMsg(null);
    const body =
      field === "design"
        ? { postcard_design_id: value || null }
        : field === "address"
          ? { return_address_id: value || null }
          : { mail_provider: value };
    try {
  ...
  async function mail() {
    ...
    try {
      ...
      if (!dry.providerConfigured) { setMsg(`${PROVIDER_LABEL[provider]} is not configured on the server.`); return; }

      const mode = dry.testMode ? " (TEST mode — no real mail/charge)" : "";
      if (!confirm(`Mail ${dry.eligible} postcard(s) via ${PROVIDER_LABEL[provider]} — est. $${dry.estimatedCostUsd.toFixed(2)}${mode}. One card per prospect, ever. Proceed?`)) return;
  ...
  const isExport = provider === "export";
  const exportHref = `/api/templates/campaigns/${campaignId}/export?mail_ready=true&do_not_mail=false`;
  ...
  return (
    <div style={panel}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-end" }}>
        <label style={fieldLabel}>Mail provider
          <select value={provider} disabled={savingField === "provider"} onChange={(e) => { const v = e.target.value as Provider; setProvider(v); assign("provider", v); }} style={select}>
            {(Object.keys(PROVIDER_LABEL) as Provider[]).map((p) => <option key={p} value={p}>{PROVIDER_LABEL[p]}</option>)}
          </select>
        </label>

        {isExport ? (
          <a href={exportHref} style={{ ...mailBtn, textDecoration: "none", display: "inline-block" }}>
            Download CSV
          </a>
        ) : (
          <>
      ...
      {isExport && (
        <p style={{ fontSize: 12, color: "#888886", marginTop: 10 }}>
          Export downloads mail-ready prospects as a CSV to import into your own mail tool. No automated send.
        </p>
      )}
      ...
      {!isExport && (designs.length === 0 || addresses.length === 0) && (
